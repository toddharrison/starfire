...
public class SimplexNoise implements NoiseGenerator {
    ...
    private static Grad grad4[] = { new Grad(0, 1, 1, 1), new Grad(0, 1, 1, -1),
            new Grad(0, 1, -1, 1), new Grad(0, 1, -1, -1), new Grad(0, -1, 1, 1),
            new Grad(0, -1, 1, -1), new Grad(0, -1, -1, 1), new Grad(0, -1, -1, -1),
            new Grad(1, 0, 1, 1), new Grad(1, 0, 1, -1), new Grad(1, 0, -1, 1),
            new Grad(1, 0, -1, -1), new Grad(-1, 0, 1, 1), new Grad(-1, 0, 1, -1),
            new Grad(-1, 0, -1, 1), new Grad(-1, 0, -1, -1), new Grad(1, 1, 0, 1),
            new Grad(1, 1, 0, -1), new Grad(1, -1, 0, 1), new Grad(1, -1, 0, -1),
            new Grad(-1, 1, 0, 1), new Grad(-1, 1, 0, -1), new Grad(-1, -1, 0, 1),
            new Grad(-1, -1, 0, -1), new Grad(1, 1, 1, 0), new Grad(1, 1, -1, 0),
    ...
    private static short p_supply[] = { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194,
            233, 7, 225, 140, 36, 103, 30, 69, 142, 8, 99, 37, 240, 21, 10, 23, 190, 6, 148, 247,
    ...
    public SimplexNoise(final long seed) {
        ...
        // the seed determines the swaps that occur between the default order and the order we're
        // actually going to use
        for (int i = 0; i < NUMBEROFSWAPS; i++) {
            final int swapFrom = rand.nextInt(p.length);
            final int swapTo = rand.nextInt(p.length);

            final short temp = p[swapFrom];
            p[swapFrom] = p[swapTo];
            p[swapTo] = temp;
        }
    ...
    public double noise(final double xin, final double yin) {
        ...
        final int ii = i & 255;
        final int jj = j & 255;
        final int gi0 = permMod12[ii + perm[jj]];
        final int gi1 = permMod12[ii + i1 + perm[jj + j1]];
        final int gi2 = permMod12[ii + 1 + perm[jj + 1]];
